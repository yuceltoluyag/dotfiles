...
ultima="/home/friday13/Pictures/wallhaven-83o2ky.jpg"
modo="--bg-fill"
indice=0
i=0
...
ajustar() {
	if [ -f "$1" ]; then
		...
		command -v feh >/dev/null 2>&1 || { notify-send -i $icone "Hata" "Feh uygulaması yüklü değil." ; exit 1; }
		feh --bg-fill "$1"
		echo "$1" > ~/.wall
	fi
}
...
[ ! -f $default ] && curl -s -L 'https://i.0imgur.com/BwOh5Z5.png' > $default
...
if [ ! $1 ]; then
	...
	notify-send -i $icone "başarı" "görüntü <b>$img</b> ova."
elif [ "$1" == "dd" ]; then
	...
	if [ "$apagar" == "Sim" ]; then
		rm $(cat ~/.wall)
		notify-send -i $icone "başarı" "görüntü <b>$(basename $(cat ~/.wall))</b> apagada."
		command -v hsetroot >/dev/null 2>&1 || { notify-send -i $icone "Hata" "HSetroot uygulaması yüklü değil." ; exit 1; }
